...
/**
  ...
  */
object targetToCompounds extends SparkJob with NamedRddSupport with Globals {
  ...
  override def runJob(sc: SparkContext, config: Config): Any = {
    ...
    val targetQuery:List[String] = Try(config.getString("query")).getOrElse("NA").split(" ").toList
    // Limit on number of results, fallback is 10
    val limit:Int = Try(config.getString("limit").toInt).getOrElse(5)
    ...
    val input = (db, genes)
    val parameters = (version, targetQuery, limit)
